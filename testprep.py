...
dataframe = pd.read_csv('mytest.csv', sep=",",header=0)
...
itemrel = traindataframe.groupby(['itemID', 'returnShipment',]).size().unstack(fill_value=0)
item_dict = {}
...
manufacturerID = traindataframe.loc[:,['manufacturerID']].values
...
customerrel = traindataframe.groupby(['customerID', 'returnShipment',]).size().unstack(fill_value=0)
customer_dict = {}
...
size = traindataframe.loc[:,['size']].values

for i in range(0,traindataframe.shape[0]):
	a = size[i][0]
	if((str(a) > '4' and str(a) < '8') or str(a) == 'xs'):
		size[i][0] = 'XS'
	elif((str(a) > '7' and str(a) < '12') or str(a) == 's'):
...
traindataframe['size'] = pd.DataFrame.from_records(size, columns = ["size"])

sizerel = traindataframe.groupby(['size','returnShipment',]).size().unstack(fill_value=0)
size_dict = {}
...
newDF.to_csv("edited_mytest.csv", sep=',', encoding='utf-8',header=True,index=False)
